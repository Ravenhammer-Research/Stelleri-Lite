/*
 * Tun system helper implementations (adapted from SystemTunnel)
 */
...
void SystemConfigurationManager::SaveTun(const TunInterfaceConfig &t) const {
  if (t.name.empty())
    throw std::runtime_error("TunInterfaceConfig has no interface name set");

  if (!t.source || !t.destination) {
    throw std::runtime_error("Tun endpoints not configured");
  }
  ...
  if (!src_addr || !dst_addr) {
    throw std::runtime_error("Invalid tun endpoint addresses");
  }
  if (src_addr->family() != dst_addr->family()) {
    throw std::runtime_error(
        "Tun endpoints must be same address family (both IPv4 or IPv6)");
  }
  ...
  if (src_addr->family() == AddressFamily::IPv4) {
    ...
    if (ioctl(tsock, SIOCSIFPHYADDR, &ifra) < 0) {
      throw std::runtime_error("Failed to configure TUN endpoints: " +
                               std::string(strerror(errno)));
    }
  } else {
    std::cerr << "Warning: IPv6 tun configuration requires routing socket - "
                 "not fully implemented\n";
  }
}

void SystemConfigurationManager::CreateTun(const std::string &nm) const {
